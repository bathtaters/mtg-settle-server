...
const { base, modelsPath, config } = require("../src.path");
const { pageOptions, actions } = require(config + "gui.cfg");
const urls = require(config + "urls.cfg").gui.basic;
const showMetrics = require(base + "libs/proxy").isEnabled;
const models = require(modelsPath).map(({ title, url }) => ({ title, url }));

exports.dbHome = (req, res) => {
  const isAdmin = hasAccess(req.user.access, access.admin);
  return res.render("dbHome", {
    title: "Home",
    user: req.user.username,
    isAdmin,
    showMetrics: isAdmin && showMetrics(),
    baseURL: urls.prefix + urls.home + "/",
    models: models
      .filter(({ title }) => req.user && hasModelAccess(req.user.models, title))
      .map(({ url }) => url),
  });
};
...
exports.modelDb = (
  ...
  const staticDbParams = {
    ...
    submitURLs: actionURLs(
      `${urls.prefix}${urls.home}/${Model.url}${urls.form}/`,
    ),
    schema: Model.schema,
  ...
  return {
    model: (req, res, next) =>
      Model.getPaginationData(matchedData(req), pageOptions)
        .then(({ data, ...pageData }) => {
          const canRead =
            req.user && hasModelAccess(req.user.models, Model.title, "read");
          const canWrite =
            req.user && hasModelAccess(req.user.models, Model.title, "write");
...
exports.userProfile = (req, res, next) => {
  const isAdmin = req.user && hasAccess(req.user.access, access.admin);
  if (!req.user) return next(errors.noData("user"));
