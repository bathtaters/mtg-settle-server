...
const getName = (err) =>
  err.name || (httpErrs[err.status] || {}).name || "Error";
const getMsg = (err) => err.message || defaultError.message;
const getCode = (err) => err.status || defaultError.status;
const formatErr = (err) =>
  err.stack || `${err.name} <${err.status}>: ${err.message}`;

// Error Middleware Stack
const catchMissing = (req, res, next) =>
  req.error ? next(req.error) : next(errors.missing());

function normalizeError(err, req, res, next) {
  req.error = err || req.error || defaultError;

  if (req.error.code === "EBADCSRFTOKEN")
    req.error = req.session ? errors.noCSRF() : errors.noSession();
