...
exports.openServer = (app, config, server) =>
  new Promise((res, rej) => {
    try {
      if (process.env.NODE_ENV === "test") return res();
...
exports.getCreds = async ({ key, cert }) => {
  ...
  if (!creds.key || !creds.cert)
    throw missingCreds(process.env.NODE_ENV !== "production");
  return creds;
};

exports.addListeners = (events, handler) =>
...
exports.closeHandler = (proc) => () => {
  ...
  if (!proc.listener || !proc.app)
    return proc.terminateServer().then(() => process.exit(0));
