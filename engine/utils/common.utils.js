...
exports.capitalizeHyphenated = (str) =>
  str
    .replace(hyphenRegex[0], (a) => a.toUpperCase())
    .replace(hyphenRegex[1], (_, g) => " " + g.toUpperCase());
...
exports.exceptRoute = (skipPath, middleware) => {
...
exports.deepEquals = (a, b, compareFunc = (a, b) => a === b) => {
  ...
  const keys = Object.keys(a),
    bKeys = Object.keys(b);
  if (keys.length !== bKeys.length || !bKeys.every((key) => keys.includes(key)))
    return false;
...
exports.splitUnenclosed = (
  ...
  const delimRegex = RegEx(
    "^" + (splitDelimiter.source || RegEx.escapeRegexPattern(splitDelimiter)),
  );
  const [open, close] = enclosures.reduce(
    (arrs, item) =>
      [0, 1].forEach((i) => {
        arrs[i] += item[i];
      }) || arrs,
  ...
  const updateCounter = (char, counter) => {
    let i;
    (i = close.indexOf(char)) > -1 && counter[i]
      ? --counter[i]
  ...
  return (text) => {
    ...
    let array = [],
      next = "";
    for (
      let match, counter = enclosures.map(() => 0);
      text;
      text = text.slice(1)
    ) {
      if (
        (match = text.match(delimRegex)) &&
        !Object.values(counter).some((n) => n)
      ) {
        if (match[0].length > 1) text = text.slice(match[0].length - 1);
        next = appendStr(next, array);
      } else if (text.charAt(0) === escape)
        ((next += text.charAt(1)), (text = text.slice(1)));
...
exports.debounce = (func, { interval = 1000, ignoreArgs = false } = {}) => {
  ...
  return [
    function debouncedFunc(...args) {
      if (
        result !== EMPTY &&
        (ignoreArgs || exports.deepEquals(args, resultargs))
      )
        return result;
...
exports.getMatchingKey = (object, propAnyCase) => {
  if (propAnyCase in object || typeof propAnyCase !== "string")
    return propAnyCase;
...
exports.caseInsensitiveObject = (object) =>
  object &&
  new Proxy(object, {
    has(object, prop) {
